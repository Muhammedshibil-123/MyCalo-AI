...
import { IoMdArrowBack, IoMdSend, IoMdCheckmarkCircle } from 'react-icons/io';
...
const DoctorChatPage = () => {
        ...
        if (data.type === 'chat_history') {
          ...
          setMessages((prev) => {
            ...
            const exists = prev.some(msg => {
              const existingTime = msg.Timestamp || msg.timestamp;
              const existingSender = msg.SenderID || msg.sender_id;
              return existingTime === msgTime && existingSender === msgSender;
            });
  ...
  const handleResolve = async () => {
    ...
    const confirmed = window.confirm('Mark this consultation as resolved? The chat will be archived.');
    if (!confirmed) return;
    ...
    try {
      ...
      alert("Failed to resolve consultation. Please try again.");
    } finally {
        ...
        <div className="flex items-center gap-3">
          ...
          {!isResolved && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-green-600 text-white font-bold text-sm hover:bg-green-700 transition-all active:scale-95 disabled:opacity-50 shadow-md"
            >
              {isResolving ? (
                <>
      ...
      {isResolved && (
        <div className="bg-green-50 border-b border-green-100 px-4 py-3 text-center">
          <p className="text-sm font-medium text-green-700">
            ✓ This consultation has been marked as resolved
          </p>
        </div>
      )}
      ...
      <div className="bg-white border-t border-gray-200 p-4 safe-area-pb">
        {isResolved ? (
          <div className="text-center py-3 text-gray-500 text-sm font-medium">
            This consultation is resolved. Messages are disabled.
          </div>
        ) : (
