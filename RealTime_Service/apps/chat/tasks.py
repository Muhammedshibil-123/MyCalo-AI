...
def process_file_upload(file_path, room_name, user_id, original_filename):
    try:
        ...
        is_media = any(
            ...
            for ext in [".mp3", ".wav", ".ogg", ".m4a", ".webm", ".mp4", ".mov", ".avi"]
        )
        resource_type = "video" if is_media else "image"

        print(
            f"🔄 Celery: Starting upload for {original_filename} as {resource_type}..."
        )

        upload_data = cloudinary.uploader.upload(
            file_path, resource_type=resource_type, folder="chat_media"
        )
        ...
        if (
            file_format in ["mp3", "wav", "ogg", "webm", "m4a"]
            or "audio" in original_filename
        ):
