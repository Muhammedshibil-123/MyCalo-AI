...
class ChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        await self.send(
            text_data=json.dumps(
                {"type": "chat_history", "messages": history}, cls=DecimalEncoder
            )
        )
    ...
    async def disconnect(self, close_code):
        ...
        await self.channel_layer.group_send(
            self.room_group_name, {"type": "call_ended", "sender_id": self.user_id}
        )
    ...
    async def call_ended(self, event):
        await self.send(
            text_data=json.dumps(
                {"type": "call_ended", "sender_id": event["sender_id"]}
            )
        )
    ...
    async def receive(self, text_data):
        ...
        await self.save_message_to_dynamo(
            self.user_id, message_text, file_url, file_type, timestamp
        )
    ...
    def save_message_to_dynamo(
        self, sender_id, message, file_url=None, file_type="text", timestamp=None
    ):
        try:
