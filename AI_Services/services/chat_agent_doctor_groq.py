...
class PatientNutritionHistoryInput(BaseModel):
    user_query: str = Field(description="The exact query about the patient's food, meals, calories, macros, or exercises.")
    user_id: int = Field(description="The ID of the patient (user_id). Always pass this exactly as provided.")

class AppNavigationInput(BaseModel):
    query: str = Field(description="The doctor's question about platform guidelines, medical disclaimers, or app usage.")
...
class DoctorGroqAgent:
    ...
    def _init_database(self):
        ...
        if self.db is None:
            try:
                ...
                print("[DATABASE] ✓ Successfully connected to PostgreSQL for Doctor Agent")
                return True
    ...
    def _init_vectorstore(self):
        ...
        if self.vectorstore is None:
            try:
                ...
                print("[CHROMA DB] ✓ Successfully connected Vector Store for Doctor Agent")
                return True
            except Exception as e:
    ...
    def _create_agent(self):
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", """You are the MyCalo Clinical AI Assistant designed to help Doctors.
    ...
    async def process_query(self, user_query: str, user_id: int) -> str:
        ...
        try:
            ...
            if "fetch_patient_nutrition_history" not in str(response) and "search_platform_guidelines" not in str(response):
